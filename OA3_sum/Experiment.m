...
Num_n=sum(data(:,1)==-1);    
m           = n;             
options.alpha_p = 0.5;          % positive tradeoff for weighted sum
options.alpha_n = 0.5;          % negative tradeoff for weighted sum
options.delta = 1;  
options.gamma = 1; 
...
options.rho=(options.alpha_p*Num_n)/(options.alpha_n*Num_p);      

options.eta =10;     % learning rate for OA3: see the paper for recommended values
...
[classifier, summ, sen, spe, acc, que, run_time, TMs, P,idx] = OA3_diag(Y,X,options,ID);
...
parfor i=1:20,
    ...
    [classifier, summ, sen, spe, acc, que, run_time, TMs, P,idx] = OA3(Y, X, options, ID);
    acc_OA3(i) = acc*100;
    time_OA3(i) = run_time;   
    P_list_OA3(i,:) = P;
	sen_OA3(i)=sen*100;
...
fprintf(1,'-------------------------------------------------------------------------------\n');
fprintf('Algorithm : (sum of weighted sen and spe ,   sensitivity,      specificity,     cpu running time)\n');
fprintf('OA3  &%.3f \t$\\pm$ %.3f \t& %.3f \t$\\pm$ %.3f \t& %.3f \t$\\pm$ %.3f \t& %.3f \t\\\\\n', mean(sum_OA3), std(sum_OA3), mean(sen_OA3), std(sen_OA3),mean(spe_OA3), std(spe_OA3), mean(time_OA3));
fprintf('OA3$_{diag}$  &%.3f \t$\\pm$ %.3f \t& %.3f \t$\\pm$ %.3f \t& %.3f \t$\\pm$ %.3f \t& %.3f \t\\\\\n', mean(sum_OA3_diag), std(sum_OA3_diag), mean(sen_OA3_diag), std(sen_OA3_diag),mean(spe_OA3_diag), std(spe_OA3_diag), mean(time_OA3_diag));
fprintf(1,'-------------------------------------------------------------------------------\n');
t2 = clock; 
%fprintf('End time$ %.1f \t \\\\\n', t2);
